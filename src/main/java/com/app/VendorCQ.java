...
public class VendorCQ extends Thread implements AutoCloseable{

    private static final Log log = LogFactory.getLog(VendorCQ.class);

    private static IgniteCache<Long, VendorAgg>  vendorAggCache;//EXTERNALIZE THIS
    private static IgniteCache<Integer, Vendor>  vendorCache;

    public void run() {
        try (Ignite igClient = Ignition.ignite()) {
            vendorCache = igClient.cache(VendorCQClient.VENDOR_CACHE_NAME);
            vendorAggCache = igClient.cache(VendorCQClient.VENDOR_AGG_CACHE_NAME);
